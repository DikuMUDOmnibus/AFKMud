...
void imclog( const char *format, ... )
{
   ...
   fprintf( stderr, "%s :: %s\n", strtime, buf2 );
   to_channel( buf, "IMCLog", LEVEL_ADMIN );
...
{
   ch_printf( ch, "%s&D", color_itom( txt, ch ) );
   return;
}
...
{
   pager_printf( ch, "%s&D", color_itom( txt, ch ) );
   return;
}
...
void imc_handle_autosetup( char *source, char *servername, char *cmd, char *txt, char *encrypt )
{
   ...
   if( !strcasecmp( cmd, "accept" ) )
   {
      ...
      {
         imclog( "SHA256 Authentication has been enabled." );
      ...
      switch ( word[0] )
      {
         ...
         case 'E':
            if( !strcasecmp( word, "End" ) )
            {
               char lib_buf[LGST];
               /*
                * Adjust infobase information based on already supplied info (mud.h). -Orion
                * Modified for AFKMud use by Samson.
                */
               snprintf( lib_buf, LGST, "%s%s %s", IMC_VERSION_STRING, CODENAME, CODEVERSION );
               this_imcmud->versionid = IMCSTRALLOC( lib_buf );
               return;
            }
            break;
...
void web_imc_list( WEB_DESCRIPTOR * wdesc )
{
   ...
   snprintf( buf, MSL, "<table><tr><td colspan=\"5\"><font color=\"white\">Active muds on %s:</font></td></tr>",
             this_imcmud->network );
   ...
   {
      imcstrlcpy( urldump, this_imcmud->www, MIL );
      snprintf( buf, MSL,
   ...
   for( rin = first_rinfo; rin; rin = rin->next, count++ )
   {
      if( !str_cmp( rin->network, "unknown" ) )
         imcstrlcpy( netname, this_imcmud->network, MIL );
      else
         imcstrlcpy( netname, rin->network, MIL );
      ...
      if( rin->path && rin->path[0] != '\0' )
      {
         if( ( start = strchr( rin->path, '!' ) ) != NULL )
         {
            ...
            imcstrlcpy( serverpath, rin->path, MIL );
      }
      ...
      {
         imcstrlcpy( urldump, rin->url, MIL );
         snprintf( buf, MSL,
   ...
   imcstrlcpy( buf,
               ...
               MSL );
   send_buf( wdesc->fd, buf );
   snprintf( buf, MSL, "<tr><td colspan=\"5\">%d connections on %s found.</td></tr></table>", count, this_imcmud->network );
   send_buf( wdesc->fd, buf );
   return;
}
#endif
