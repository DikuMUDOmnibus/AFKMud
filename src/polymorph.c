...
CMDF do_morphstat( CHAR_DATA * ch, char *argument );
...
void fwrite_morph( FILE * fp, MORPH_DATA * morph )
{
   ...
   if( !xIS_EMPTY( morph->immune ) )
      fprintf( fp, "Immune          	%s\n", print_bitvector( &morph->immune ) );
   if( !xIS_EMPTY( morph->absorb ) )
      fprintf( fp, "Absorb			%s\n", print_bitvector( &morph->absorb ) );
   if( !xIS_EMPTY( morph->no_affected_by ) )
      fprintf( fp, "NoAffected      	%s\n", print_bitvector( &morph->no_affected_by ) );
   if( !xIS_EMPTY( morph->no_immune ) )
      fprintf( fp, "NoImmune        	%s\n", print_bitvector( &morph->no_immune ) );
   if( !xIS_EMPTY( morph->no_resistant ) )
      fprintf( fp, "NoResistant     	%s\n", print_bitvector( &morph->no_resistant ) );
   if( !xIS_EMPTY( morph->suscept ) )
      fprintf( fp, "NoSuscept       	%s\n", print_bitvector( &morph->no_suscept ) );
   if( morph->race != 0 )
      fprintf( fp, "Race        	%s~\n", race_string( morph->race ) );
   if( !xIS_EMPTY( morph->resistant ) )
      fprintf( fp, "Resistant       	%s\n", print_bitvector( &morph->resistant ) );
   if( !xIS_EMPTY( morph->suscept ) )
      fprintf( fp, "Suscept        	%s\n", print_bitvector( &morph->suscept ) );
   if( morph->timer != 0 )
      fprintf( fp, "Timer		%d\n", morph->timer );
   if( morph->used != 0 )
      fprintf( fp, "Used       	%d\n", morph->used );
 ...
 */
CMDF do_morphset( CHAR_DATA * ch, char *argument )
{
   ...
   switch ( ch->substate )
   {
      ...
      case SUB_MORPH_DESC:
         if( !ch->pcdata->dest_buf )
         {
            ...
            bug( "%s", "do_morphset: sub_morph_desc: NULL ch->pcdata->dest_buf" );
      ...
      case SUB_MORPH_HELP:
         ...
         {
            send_to_char( "Fatal error: report to Samson.\n\r", ch );
            bug( "%s", "do_morphset: sub_morph_help: NULL ch->pcdata->dest_buf" );
            ch->substate = SUB_NONE;
            return;
         }
         morph = ( MORPH_DATA * ) ch->pcdata->dest_buf;
   ...
   else if( !str_cmp( arg2, "timeto" ) )
   {
      ...
      {
         ch_printf( ch, "Timeto is a value from 0 to %d.\n\r", sysdata.hoursperday - 1 );
         return;
      }
      morph->timeto = value;
   }
   else if( !str_cmp( arg2, "timefrom" ) )
   {
      if( value < 0 || value > sysdata.hoursperday - 1 )
      {
         ch_printf( ch, "Timefrom is a value from 0 to %d.\n\r", sysdata.hoursperday - 1 );
         return;
      }
   ...
   else if( !str_cmp( arg2, "dayto" ) )
   {
      ...
      {
         ch_printf( ch, "Dayto is a value from 0 to %d.\n\r", sysdata.dayspermonth - 1 );
         return;
      }
      morph->dayto = value;
   }
   else if( !str_cmp( arg2, "dayfrom" ) )
   {
      if( value < 0 || value > sysdata.dayspermonth - 1 )
      {
         ch_printf( ch, "Dayfrom is a value from 0 to %d.\n\r", sysdata.dayspermonth - 1 );
         return;
      }
      morph->dayfrom = value;
   }
   ...
   else if( !str_cmp( arg2, "resistant" ) || !str_cmp( arg2, "r" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         if( value < 0 || value >= MAX_RIS_FLAG )
            ...
            xTOGGLE_BIT( morph->resistant, value );
      }
   }
   else if( !str_cmp( arg2, "susceptible" ) || !str_cmp( arg2, "s" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         if( value < 0 || value >= MAX_RIS_FLAG )
            ...
            xTOGGLE_BIT( morph->suscept, value );
      }
   }
   else if( !str_cmp( arg2, "immune" ) || !str_cmp( arg2, "i" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         if( value < 0 || value >= MAX_RIS_FLAG )
            ...
            xTOGGLE_BIT( morph->immune, value );
      }
   }
   else if( !str_cmp( arg2, "absorb" ) || !str_cmp( arg2, "a" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: morphset <morph> absorb <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
         ...
         if( value < 0 || value >= MAX_RIS_FLAG )
            ...
            xTOGGLE_BIT( morph->absorb, value );
      }
   }
   else if( !str_cmp( arg2, "noresistant" ) || !str_cmp( arg2, "nr" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         if( value < 0 || value >= MAX_RIS_FLAG )
            ...
            xTOGGLE_BIT( morph->no_resistant, value );
      }
   }
   else if( !str_cmp( arg2, "nosusceptible" ) || !str_cmp( arg2, "ns" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         if( value < 0 || value >= MAX_RIS_FLAG )
            ...
            xTOGGLE_BIT( morph->no_suscept, value );
      }
   }
   else if( !str_cmp( arg2, "noimmune" ) || !str_cmp( arg2, "ni" ) )
   {
      ...
      while( argument[0] != '\0' )
      {
         ...
         if( value < 0 || value >= MAX_RIS_FLAG )
            ...
            xTOGGLE_BIT( morph->no_immune, value );
      }
   }
...
CMDF do_morphstat( CHAR_DATA * ch, char *argument )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
      ...
      pager_printf( ch, " &cCasting Allowed   : &C%s\n\r", ( morph->cast_allowed ? "Yes" : "No" ) );
      ...
      pager_printf( ch, "  &cImmune     : &w%s\n\r", ext_flag_string( &morph->immune, ris_flags ) );
      pager_printf( ch, "  &cSusceptible: &w%s\n\r", ext_flag_string( &morph->suscept, ris_flags ) );
      pager_printf( ch, "  &cResistant  : &w%s\n\r", ext_flag_string( &morph->resistant, ris_flags ) );
      pager_printf( ch, "  &cAbsorb     : &w%s\n\r", ext_flag_string( &morph->absorb, ris_flags ) );
      ...
      pager_printf( ch, "  &cNot Immune     : &w%s\n\r", ext_flag_string( &morph->no_immune, ris_flags ) );
      pager_printf( ch, "  &cNot Susceptible: &w%s\n\r", ext_flag_string( &morph->no_suscept, ris_flags ) );
      pager_printf( ch, "  &cNot Resistant  : &w%s\n\r", ext_flag_string( &morph->no_resistant, ris_flags ) );
 ...
 * --Shaddai
 */
CMDF do_morph( CHAR_DATA * ch, MORPH_DATA * morph )
{
   CHAR_MORPH *ch_morph;
   ...
   xSET_BITS( ch->immune, morph->immune );
   xSET_BITS( ch->resistant, morph->resistant );
   xSET_BITS( ch->susceptible, morph->suscept );
   xSET_BITS( ch->absorb, morph->absorb );
   xREMOVE_BITS( ch->affected_by, morph->no_affected_by );
   xREMOVE_BITS( ch->immune, morph->no_immune );
   xREMOVE_BITS( ch->resistant, morph->no_resistant );
   xREMOVE_BITS( ch->susceptible, morph->no_suscept );
   ch->morph = ch_morph;
   morph->used++;
   return;
}
...
CMDF do_unmorph( CHAR_DATA * ch )
{
   ...
   xREMOVE_BITS( ch->immune, morph->immune );
   xREMOVE_BITS( ch->resistant, morph->resistant );
   xREMOVE_BITS( ch->susceptible, morph->suscept );
   xREMOVE_BITS( ch->absorb, morph->absorb );
   DISPOSE( ch->morph );
   update_aris( ch );
   return;
}
...
void morph_defaults( MORPH_DATA * morph )
{
   ...
   morph->hpused = 0;
   morph->favourused = 0;
   xCLEAR_BITS( morph->immune );
   xCLEAR_BITS( morph->absorb );
   xCLEAR_BITS( morph->no_affected_by );
   xCLEAR_BITS( morph->no_immune );
   xCLEAR_BITS( morph->no_resistant );
   xCLEAR_BITS( morph->no_suscept );
   morph->obj[0] = 0;
   morph->obj[1] = 0;
   morph->obj[2] = 0;
   ...
   morph->race = 0;
   xCLEAR_BITS( morph->resistant );
   xCLEAR_BITS( morph->suscept );
   morph->used = 0;
   morph->ac = 0;
...
MORPH_DATA *fread_morph( FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      {
         case 'A':
            KEY( "Absorb", morph->absorb, fread_bitvector( fp ) );
         ...
         case 'I':
            ...
            KEY( "Immune", morph->immune, fread_bitvector( fp ) );
            break;
         case 'K':
         ...
         case 'N':
            ...
            KEY( "NoImmune", morph->no_immune, fread_bitvector( fp ) );
            KEY( "NoResistant", morph->no_resistant, fread_bitvector( fp ) );
            MKEY( "NoSkills", morph->no_skills, fread_string_nohash( fp ) );
            KEY( "NoSuscept", morph->no_suscept, fread_bitvector( fp ) );
         ...
         case 'R':
            ...
            KEY( "Resistant", morph->resistant, fread_bitvector( fp ) );
            break;
         case 'S':
            ...
            KEY( "Suscept", morph->suscept, fread_bitvector( fp ) );
            break;
         case 'T':
 ...
 */
CMDF do_morphcreate( CHAR_DATA * ch, char *argument )
{
 ...
 */
CMDF do_morphdestroy( CHAR_DATA * ch, char *argument )
{
   MORPH_DATA *morph;
 ...
 */
CMDF do_imm_morph( CHAR_DATA * ch, char *argument )
{
   MORPH_DATA *morph;
 ...
 */
CMDF do_imm_unmorph( CHAR_DATA * ch, char *argument )
{
   CHAR_DATA *victim = NULL;
...
CMDF do_morphlist( CHAR_DATA * ch, char *argument )
{
   MORPH_DATA *morph;
