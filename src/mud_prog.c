...
void rprog_wordlist_check( char *arg, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, int type,
...
void init_supermob( void )
{
   ...
   if( !char_to_room( supermob, office ) )
...
bool mprog_seval( char *lhs, char *opr, char *rhs, CHAR_DATA * mob )
{
   ...
   progbugf( mob, "mprog_seval: Improper MOBprog operator '%s'", opr );
   return 0;
}
 ...
 doneargs:
   ...
   *p = '\0';
   ...
   if( chkchar )
   {
      ...
      {
         return ( IS_ACT_FLAG( chkchar, ACT_PACIFIST ) );
      }
      if( !str_cmp( chck, "ismobinvis" ) )
      {
         return ( IS_ACT_FLAG( chkchar, ACT_MOBINVIS ) );
      }
      ...
      {
         return IS_ROOM_FLAG( chkchar->in_room, ROOM_NO_RECALL ) ? TRUE : FALSE;
      }
      if( !str_cmp( chck, "sex" ) )
      {
      ...
      if( !str_cmp( chck, "speaking" ) )
      {
         for( lang = 0; lang_array[lang] != LANG_UNKNOWN; lang++ )
            if( chkchar->speaking & lang_array[lang] )
      ...
      {
         for( lang = 0; lang_array[lang] != LANG_UNKNOWN; lang++ )
            if( IS_SET( chkchar->speaks, lang_array[lang] ) )
               return mprog_seval( lang_names[lang], opr, rval, mob );
...
*/
   ...
   if( !str_cmp( chck, "mortinworld" ) )  /* -- Gorog */
   {
      ...
      for( d = first_descriptor; d; d = d->next )
         if( d->connected == CON_PLAYING && d->character && get_trust( d->character ) < LEVEL_IMMORTAL
   ...
   if( !str_cmp( chck, "immortinworld" ) )   /* -- Samson */
   {
      ...
      {
         if( IS_NPC( tmp ) || !IS_IMMORTAL( tmp ) )
            continue;

         if( nifty_is_name( tmp->name, cvar ) )
            return TRUE;
      }
      return FALSE;
   }
   ...
   if( !str_cmp( chck, "mortinroom" ) )   /* -- Gorog */
   {
      ...
      for( ch = mob->in_room->first_person; ch; ch = ch->next_in_room )
         if( ( !IS_NPC( ch ) ) && ch->level < LEVEL_IMMORTAL && nifty_is_name( ch->name, cvar ) )
            return TRUE;
      return FALSE;
   }
   ...
   if( !str_cmp( chck, "mortinarea" ) )   /* -- Gorog */
   {
      CHAR_DATA *ch;
      for( ch = first_char; ch; ch = ch->next )
         if( ( !IS_NPC( ch ) ) && ch->in_room->area == mob->in_room->area
 ...
 */
void mprog_and_wordlist_check( char *arg, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, int type )
{
   char temp1[MSL];
   char temp2[MIL];
   char word[MIL];
...
void oprog_and_wordlist_check( char *arg, CHAR_DATA * mob, CHAR_DATA * actor,
...
void rprog_and_wordlist_check( char *arg, CHAR_DATA * mob, CHAR_DATA * actor,
...
bool mprog_keyword_trigger( char *txt, CHAR_DATA * actor )
{
   ...
   {
      for( mprg = actor->in_room->mudprogs; mprg; mprg = mprg->next )
      {
         if( char_died( actor ) )
            return rValue;
         ...
         if( ( mprg->type = KEYWORD_PROG ) && ( !str_cmp( mprg->arglist, txt ) ) )
         {
            ...
            mprog_driver( mprg->comlist, supermob, actor, NULL, NULL, FALSE );
            release_supermob(  );
            rValue = TRUE;
         }
      }
   ...
   {
      if( IS_NPC( vmob ) && HAS_PROG( vmob->pIndexData, KEYWORD_PROG ) )
      {
         ...
         for( mprg = vmob->pIndexData->mudprogs; mprg; mprg = mprg->next )
         {
            ...
            if( ( mprg->type == KEYWORD_PROG ) && ( !str_cmp( mprg->arglist, txt ) ) )
            {
               mprog_driver( mprg->comlist, vmob, actor, NULL, NULL, FALSE );
               rValue = TRUE;
            }
         }
      }
   ...
   for( vobj = actor->in_room->first_content; vobj; vobj = vobj->next_content )
   {
      if( HAS_PROG( vobj->pIndexData, KEYWORD_PROG ) )
      {
         for( mprg = vobj->pIndexData->mudprogs; mprg; mprg = mprg->next )
         {
            ...
            {
               set_supermob( vobj );
               mprog_driver( mprg->comlist, supermob, actor, vobj, NULL, FALSE );
               release_supermob(  );
               rValue = TRUE;
            }
         }
   ...
   for( vobj = actor->first_carrying; vobj; vobj = vobj->next_content )
   {
      if( HAS_PROG( vobj->pIndexData, KEYWORD_PROG ) )
      {
         for( mprg = vobj->pIndexData->mudprogs; mprg; mprg = mprg->next )
...
void mprog_and_speech_trigger( char *txt, CHAR_DATA * actor )
{
   CHAR_DATA *vmob;

   for( vmob = actor->in_room->first_person; vmob; vmob = vmob->next_in_room )
   {
      if( IS_NPC( vmob ) && HAS_PROG( vmob->pIndexData, SPEECH_AND_PROG ) )
      {
         if( IS_NPC( actor ) && actor->pIndexData == vmob->pIndexData )
            continue;
         mprog_and_wordlist_check( txt, vmob, actor, NULL, NULL, SPEECH_AND_PROG );
      }
   }
}
...
{
   if( IS_NPC( victim ) && HAS_PROG( victim->pIndexData, SPEECH_PROG ) )
   {
      if( IS_NPC( actor ) && actor->pIndexData == victim->pIndexData )
         return;
      mprog_wordlist_check( txt, victim, actor, NULL, NULL, SPEECH_PROG );
   }
}
...
void oprog_and_speech_trigger( char *txt, CHAR_DATA * ch )
{
   ...
   for( vobj = ch->in_room->first_content; vobj; vobj = vobj->next_content )
      if( HAS_PROG( vobj->pIndexData, SPEECH_AND_PROG ) )
         oprog_and_wordlist_check( txt, supermob, ch, vobj, NULL, SPEECH_AND_PROG, vobj );
...
void rprog_and_speech_trigger( char *txt, CHAR_DATA * ch )
{
   if( HAS_PROG( ch->in_room, SPEECH_AND_PROG ) )
   {
       ...
       */
      rprog_and_wordlist_check( txt, supermob, ch, NULL, NULL, SPEECH_AND_PROG, ch->in_room );
   }
}
...
*                                                                         *
**************************************************************************/
void mprog_month_check( CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, int type )
{
   MPROG_DATA *mprg;
...
void rprog_month_check( CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, int type )
{
   ROOM_INDEX_DATA *room = ( ROOM_INDEX_DATA * ) vo;
   MPROG_DATA *mprg;
...
void oprog_month_check( CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, int type )
{
   MPROG_DATA *mprg;
...
void progbugf( CHAR_DATA * mob, char *fmt, ... )
{
   char buf[MSL * 2];
   va_list args;
...
void progbug( char *str, CHAR_DATA * mob )
{
   ...
   if( vnum == MOB_VNUM_SUPERMOB )
   {
       ...
       */
      bug( "%s, %s.", str, mob->chardesc == NULL ? "(unknown)" : mob->chardesc );
   }
   else
